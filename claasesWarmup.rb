class Kardashian
  ...
  def initialize(name, babies, birth_year)
    @name = name
    @babies = babies #question type one
    @birth_year = birth_year #question type two
  end #def end
...
class Quiz
  ...
  @@kardashian_questions = [] #array of questions
  @@total_questions = 0 #total question counter
  @@correct_answers = 0 #correct answer counter
 
  def self.add_kardashian(kardashian) #this is where we add each object to the array
  @@kardashian_questions << kardashian 
  @@total_questions += 1 #adds to the number of questions each time
  end #def end
  ...
  @@kardashian_questions.each do |kardashian|
    self.quiz_question_babies(kardashian) #number of baby(s) questions
    self.quiz_questions_birth_year(kardashian) #birth year questions
  end #loop end
